...
{
    char buffer[256];
    const size_t len = jwin_event_to_str(sizeof(buffer), buffer, e);
...
int main()
{
    ...
    for (unsigned i = 0; i < WINDOW_COUNT; ++i)
    {
        ...
        ASSERT(res == JWIN_RESULT_SUCCESS);
        JWIN_TEST_CALL(jwin_window_set_event_handler(wnd_array[i], JWIN_EVENT_TYPE_DESTROY, (jwin_event_callback){ .destroy = destroy_callback }, "Banana"));
        ASSERT(res == JWIN_RESULT_SUCCESS);
    }
    ...
    const unsigned window_close_array[5] = { 2, 5, 6, 1, 9 };
    unsigned i = 0;
